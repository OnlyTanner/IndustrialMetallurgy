...
public class BlockChemicalReactor extends BlockContainer {

    public static final PropertyDirection FACING = BlockHorizontal.FACING;
    private final boolean IS_BURNING;
    private static boolean keepInventory;

    public BlockChemicalReactor(boolean isBurning, String unlocalizedName, String registryName) {
        super(Material.IRON);
    ...
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return Item.getItemFromBlock(ModBlocks.chemical_reactor);
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            playerIn.openGui(IndustrialMetallurgy.instance, GuiHandlerChemicalReactor.getGuiID(), worldIn, pos.getX(), pos.getY(), pos.getZ());
        }
        return true;
    }
...
/*
        if (active) {
            worldIn.setBlockState(pos, ModBlocks.lit_forge_tier2.getDefaultState().withProperty(FACING, blockState.getValue(FACING)), 3);
            worldIn.setBlockState(pos, ModBlocks.lit_forge_tier2.getDefaultState().withProperty(FACING, blockState.getValue(FACING)), 3);
        } else {*/
            worldIn.setBlockState(pos, ModBlocks.chemical_reactor.getDefaultState().withProperty(FACING, blockState.getValue(FACING)), 3);
            worldIn.setBlockState(pos, ModBlocks.chemical_reactor.getDefaultState().withProperty(FACING, blockState.getValue(FACING)), 3);
        //}
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        if (!keepInventory) {
            ...
            if (tileentity instanceof TileEntityChemicalReactor) {
                InventoryHelper.dropInventoryItems(worldIn, pos, (TileEntityChemicalReactor) tileentity);
    ...
    public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
        return new ItemStack(ModBlocks.chemical_reactor);
    }

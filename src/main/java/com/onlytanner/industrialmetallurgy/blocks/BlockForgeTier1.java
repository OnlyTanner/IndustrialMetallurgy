...
public class BlockForgeTier1 extends BlockContainer {

    public static final PropertyDirection FACING = BlockHorizontal.FACING;
    private final boolean IS_BURNING;
    private static boolean keepInventory;

    public BlockForgeTier1(boolean isBurning, String unlocalizedName, String registryName) {
        super(Material.IRON);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            playerIn.openGui(IndustrialMetallurgy.instance, GuiHandlerForgeTier1.getGuiID(), worldIn, pos.getX(), pos.getY(), pos.getZ());
        }
        return true;
    }
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        if (!keepInventory) {
            ...
            if (tileentity instanceof TileEntityForgeTier1) {
                InventoryHelper.dropInventoryItems(worldIn, pos, (TileEntityForgeTier1) tileentity);

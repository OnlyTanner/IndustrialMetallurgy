...
public class ExtruderTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider, IEnergyStorage {
    ...
    public ExtruderTileEntity() {
        this(ModTileEntityTypes.EXTRUDER.get());
    }
    ...
    public Container createMenu(final int windowID, final PlayerInventory playerInv, final PlayerEntity playerIn) {
        return new ExtruderContainer(windowID, playerInv, this);
    }
    ...
    public void tick() {
        ...
        if (this.world != null && !this.world.isRemote) {
            ...
            if (this.getRecipe() != null && canProcess() && this.inventory.getStackInSlot(BURR_SET_ID).getCount() > 0 && energy >= ENERGY_USAGE_PER_TICK) {
                this.world.setBlockState(this.getPos(), this.getBlockState().with(ExtruderBlock.LIT, true));
                ...
                this.world.setBlockState(this.getPos(), this.getBlockState().with(ExtruderBlock.LIT, false));
                currentSmeltTime = 0;
            }
        }
        if (dirty) {
    ...
    public ITextComponent getDefaultName() {
        return new TranslationTextComponent("container." + IndustrialMetallurgy.MOD_ID + ".extruder");
    }
    ...
    @Nullable
    private ExtruderRecipe getRecipe() {
        if (this.inventory == null)
            return null;
        Set<IRecipe<?>> recipes = findRecipesByType(RecipeSerializerInit.EXTRUDER_RECIPE_RECIPE_TYPE, this.world);
        for (IRecipe<?> iRecipe : recipes) {
            ExtruderRecipe recipe = (ExtruderRecipe) iRecipe;

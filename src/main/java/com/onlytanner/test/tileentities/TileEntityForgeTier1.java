...
public class TileEntityForgeTier1 extends TileEntityLockable implements IInventory, ITickable
{
	...
	private boolean getResult(int index, int numOfOutputs, boolean performSmelt)
	{
		...
		if ((itemStacks[FIRST_INPUT_SLOT] == null && itemStacks[FIRST_INPUT_SLOT + 1] == null) || result == null) return false;
		if (!performSmelt &&
		   ...
		   (itemStacks[FIRST_OUTPUT_SLOT + 1] != null && itemStacks[FIRST_OUTPUT_SLOT + 1].isItemEqual(result)))) return true;
		...
		if (numOfOutputs == 1)
			...
			if (itemStacks[FIRST_INPUT_SLOT + index].stackSize <= 1 && itemStacks[FIRST_INPUT_SLOT + index + 1].stackSize <= 1)
			{
				itemStacks[FIRST_INPUT_SLOT + index] = null;
